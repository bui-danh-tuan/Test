...
def get_vector(text_str):
    try:
        input_ids = tokenizer(text_str, return_tensors="pt", truncation=True, padding=True, max_length=512)["input_ids"].to(device)
        with torch.no_grad():
...
def build_faiss_index(data):
    ...
    if new_ids:
        try:
            session.execute(
                text("UPDATE uet_chunking SET vector = 1 WHERE id IN :ids").bindparams(
                    bindparam("ids", expanding=True)
                ),
                {"ids": tuple(new_ids)}  # phải là tuple để dùng IN
            )
            session.commit()
        except Exception as e:
            print(f"Lỗi cập nhật vector cho các ID: {e}")
...
def get_id_by_text(text_str):
    index = faiss.read_index(faiss_has_accent_path)
    with open(faiss_ids_path, 'rb') as f:
        id_map = pickle.load(f)
    rev_id_map = {v: k for k, v in id_map.items()}

    query_vec = get_vector(text_str).reshape(1, -1).astype('float32')
...
if __name__ == "__main__":
    result = session.execute(text("SELECT id, title, content FROM uet_chunking WHERE vector = 0 and CHARACTER_LENGTH(content) > 100"))
    data = []
    list_data = []
    count = 0
    number_split = 1000
    for row in result:
        id_chunking, title, content = row
        data.append({
            'id': id_chunking,
            'text': content
        })
